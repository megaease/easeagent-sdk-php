...
namespace Easeagent\Reporters\Http;
...
final class CurlFactory implements ClientFactory
{
    ...
    public function build(array $options): callable
    {
        ...
        return static function (string $payload) use ($options): void {
            ...
            if ($options['enable_tls'] == true) {
                \curl_setopt($handle, CURLOPT_SSLKEY_BLOB, $options['tls_key']);
                // The --cert option
                \curl_setopt($handle, CURLOPT_SSLCERT_BLOB, $options['tls_cert']);
            }
